...
describe Workday::Client do
    ...
    let(:credentials) {{username: "test@tenant", password: "password", tenant: "test"}}
    let(:client){ Workday::Client.new(credentials) }
    
    describe "#operations" do
        it "should return list of operations from the wsdl" do
            expect(client.operations).to be_an(Array)
            expect(client.operations).to include(:put_location, :find_business_site, :get_organization, :add_update_organization)
        end
    end
